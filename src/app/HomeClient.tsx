...
function Hero() {
  ...
  return (
    ...
    >
      {/* ── LEFT: Vidéo hero — bord à bord, autoplay muet bouclé ── */}
      <div
        ref={mediaRef}
        className="relative w-full md:w-1/2 h-[50vh] md:h-full overflow-hidden bg-charbon"
      >
        <video
          src="/video/leslie-hero.mp4"
          autoPlay
          muted
          loop
          playsInline
          preload="auto"
          aria-label="Leslie Folcarelli dansant le tango argentin"
          className="absolute inset-0 w-full h-full object-cover"
        />

        {/* Subtle overlay text on media */}
        <div className="absolute bottom-8 left-8 right-8 pointer-events-none">
          <p className="text-[10px] tracking-[3px] uppercase text-blanc/70 [text-shadow:0_1px_2px_rgba(0,0,0,0.4)]">
...
function AboutPreview({ aboutPhoto, aboutAlt }: { aboutPhoto: string | null; aboutAlt: string }) {
  return (
...
function Events({ events }: { events: HomeEvent[] }) {
  const containerRef = useRef<HTMLDivElement>(null);
  ...
  // Adapt grid columns to event count for visual balance
  const gridClass =
    events.length === 1
      ? "grid grid-cols-1 max-w-md mx-auto gap-5"
      : events.length === 2
      ? "grid grid-cols-1 md:grid-cols-2 max-w-3xl mx-auto gap-5"
      ...
      <div className="max-w-[min(1200px,90vw)] mx-auto">
        ...
        <div ref={containerRef} className={gridClass}>
          {events.map((event) => (
            ...
            >
              <div className="font-display text-[clamp(20px,2vw,28px)] font-light text-dore leading-none mb-4">
...
export default function HomeClient({ aboutPhoto, aboutAlt, events }: Props) {
  return (
    <>
      <Hero />
      <Portals />
      <AboutPreview aboutPhoto={aboutPhoto} aboutAlt={aboutAlt} />
